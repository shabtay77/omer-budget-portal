...
// פונקציית המרת תאריך אקסל לתאריך ישראלי
...
const App = () => {
  ...
  const downloadWorkPlanExcel = () => {
    ...
    const rows = filteredWorkData.filter(t => t.id).map(t => [
          ...
          <div className="flex flex-col h-full bg-white">
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                {currentUser.role === 'ADMIN' ? (
                  ...
                  <div className="w-full flex items-center justify-start gap-3 p-4 rounded-2xl bg-emerald-50 text-emerald-800 font-black">{React.createElement(ICONS[activeWingId] || Building2, { size: 20 })} <span>{activeWingId}</span></div>
                )}
             </div>
          </div>
        </aside>
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-x-hidden text-right">
          <div className="max-w-[1400px] mx-auto">
            <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              {mainTab === 'workplan' && <button onClick={downloadWorkPlanExcel} className="flex items-center gap-2 bg-slate-800 text-white px-6 py-3 rounded-2xl font-bold text-sm shadow-lg hover:scale-105 transition-all"><Download size={20} /> הורד אקסל מלא</button>}
            </header>
            ...
            {mainTab === 'budget' ? (
              <div className="space-y-8">
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8 text-right">
                   ...
                   <div className="bg-white p-4 rounded-2xl shadow-sm border-b-4 border-slate-400"><p className="text-[8px] font-bold text-slate-400 uppercase text-right">ביצוע 26</p><p className="text-lg font-black text-slate-900">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+(c.a2026||0),0))}</p></div>
                </div>
                {viewMode === 'dashboard' ? (
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-right">
                      ...
                      <div className="bg-white p-8 rounded-[3rem] shadow-sm border min-h-[400px] text-right">
                          ...
                          <div className="h-[300px] text-right"><ResponsiveContainer width="100%" height="100%"><BarChart data={Array.from(new Set(filteredWorkData.map(t => t.dept))).map(d => ({ n: d, v: filteredWorkData.filter(t => t.dept === d).length }))} layout="vertical" margin={{ left: 30, right: 30 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} opacity={0.1} /><XAxis type="number" hide /><YAxis dataKey="n" type="category" width={120} tick={{fontSize: 10, fontWeight: 'black', fill: '#1e293b'}} axisLine={false} tickLine={false} /><Tooltip /><Bar dataKey="v" radius={[0, 10, 10, 0]} barSize={25} fill="#3b82f6" /></BarChart></ResponsiveContainer></div>
                      </div>
                    </div>
                  </>
                ) : (
                  <div className="space-y-4 text-right text-right">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-4 rounded-3xl shadow-sm border items-end text-right">
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400">חיפוש משימה</label><input type="text" placeholder="חפש..." value={search} onChange={e => setSearch(e.target.value)} className="w-full p-2 bg-slate-50 border rounded-xl outline-none font-bold text-sm text-right" /></div>
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400">מחלקה</label><select value={filterDept} onChange={e => setFilterDept(e.target.value)} className="w-full p-2 lg:p-3 rounded-xl bg-slate-50 border outline-none font-bold text-sm text-right text-slate-900" disabled={currentUser?.role === 'DEPT'}><option value="הכל">כל המחלקות</option>{Array.from(new Set(filteredWorkData.map(t=>t.dept))).map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400">סטטוס</label><select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full p-2 lg:p-3 rounded-xl bg-slate-50 border outline-none font-bold text-sm text-right text-slate-900"><option value="הכל">כל הסטטוסים</option><option value="1">בוצע</option><option value="2">בתהליך</option><option value="3">לא בוצע</option><option value="missing">טרם עודכן</option></select></div>
                    </div>
                        ...
                        <table className="w-full text-right border-collapse min-w-[1000px] text-right text-right">
                            ...
                            <tbody className="divide-y text-right">
                                {filteredWorkData.map(task => (
                                    <tr key={task.id} className={`hover:bg-slate-50 transition-all ${task.rating === 1 ? 'bg-emerald-50/20' : ''} text-right`}>
