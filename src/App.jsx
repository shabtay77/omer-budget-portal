...
const App = () => {
      ...
      try {
        ...
        rows.slice(1).forEach(cols => {
          ...
          if (id) {
            ...
            execMap[cleanId] = parseFloat(String(cols[a26Idx] || "0").replace(/,/g, '')) || 0;
        ...
        setWorkPlans(wJson.filter(t => t.id).map(t => ({ ...t, rating: liveStatus[String(t.id)] || 0 })));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert("השינויים נשמרו בשיטס! הנתונים יתעדכנו באתר תוך כדקה.");
      setPendingChanges([]);
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-white font-black text-emerald-800" dir="rtl">טוען נתונים חיים...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-right" dir="rtl">
      ...
      <div className="flex flex-1">
        <aside className={`${isMenuOpen ? 'fixed inset-0 z-[600] flex' : 'hidden'} lg:static lg:block lg:w-72 bg-white border-l h-screen lg:h-[calc(100vh-64px)]`}>
          <div className="bg-white w-72 h-full flex flex-col p-6 shadow-xl lg:shadow-none">
            <button onClick={()=>setIsMenuOpen(false)} className="lg:hidden self-end mb-4 p-2 bg-slate-100 rounded-full"><X size={20}/></button>
            <div className="space-y-2 mb-8">
              <button onClick={()=>{setViewMode('dashboard'); setIsMenuOpen(false);}} className={`w-full flex items-center gap-3 p-4 rounded-2xl ${viewMode === 'dashboard' ? 'bg-emerald-800 text-white' : 'text-slate-600 hover:bg-emerald-50'}`}><LayoutDashboard size={20}/> <span className="font-bold">תמונת מצב</span></button>
              <button onClick={()=>{setViewMode('table'); setIsMenuOpen(false);}} className={`w-full flex items-center gap-3 p-4 rounded-2xl ${viewMode === 'table' ? 'bg-emerald-800 text-white' : 'text-slate-600 hover:bg-emerald-50'}`}><TableProperties size={20}/> <span className="font-bold">פירוט מלא</span></button>
            </div>
            <p className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest">אגפים</p>
            <div className="overflow-y-auto flex-1 space-y-1">
              {currentUser.role === 'ADMIN' ? (
                <>
                  <button onClick={()=>{setActiveWingId(null); setIsMenuOpen(false);}} className={`w-full text-right p-3 rounded-xl font-bold ${!activeWingId ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>כלל המועצה</button>
          ...
          {mainTab === 'budget' ? (
            <div className="space-y-8">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-3xl border shadow-sm"><p className="text-[10px] font-bold text-slate-400">תקציב 2026</p><p className="text-xl font-black text-emerald-800">{formatILS(budgetStats.total)}</p></div>
                <div className="bg-white p-6 rounded-3xl border shadow-sm"><p className="text-[10px] font-bold text-slate-400">ביצוע בפועל</p><p className="text-xl font-black text-blue-800">{formatILS(budgetStats.exec)}</p></div>
                <div className="bg-white p-6 rounded-3xl border shadow-sm"><p className="text-[10px] font-bold text-slate-400">תקציב 2025</p><p className="text-xl font-black text-slate-500">{formatILS(budgetStats.b25)}</p></div>
                <div className="bg-white p-6 rounded-3xl border shadow-sm"><p className="text-[10px] font-bold text-slate-400">ביצוע 2024</p><p className="text-xl font-black text-slate-500">{formatILS(budgetStats.a24)}</p></div>
              </div>
              {viewMode === 'dashboard' ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[400px]">
                  <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                    <h3 className="font-black mb-4 border-r-4 border-emerald-500 pr-2">התפלגות לפי אגפים</h3>
                    <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={Object.keys(ICONS).map(w => ({name:w, value: staticData.filter(r=>r.wing===w && r.type==='הוצאה').reduce((a,c)=>a+(parseFloat(String(c.b2026).replace(/,/g,''))||0),0)})).filter(v=>v.value>0)} dataKey="value" innerRadius={60} outerRadius={100} paddingAngle={5}>{[ '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899' ].map((c,i)=><Cell key={i} fill={c}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
                  </div>
                  <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                    <h3 className="font-black mb-4 border-r-4 border-blue-500 pr-2">תקציב מול ביצוע</h3>
                ...
                <div className="bg-white rounded-3xl border shadow-sm overflow-x-auto p-4">
                  <table className="w-full text-right">
                    <thead><tr className="bg-slate-50 text-[10px] font-black border-b uppercase"><th className="p-4">סעיף</th><th>תיאור</th><th>מחלקה</th><th className="text-left">תקציב</th><th className="text-left">ביצוע</th></tr></thead>
                    <tbody className="divide-y">{staticData.filter(r => (!activeWingId || r.wing === activeWingId) && (!search || r.name?.includes(search))).map(row => (
                      <tr key={row.id} className="hover:bg-slate-50"><td className="p-4 font-mono text-[10px] text-slate-400">#{row.id}</td><td className="p-4 font-black text-xs">{row.name}</td><td className="p-4 text-xs font-bold">{row.dept}</td><td className="p-4 text-left font-bold text-emerald-800">{formatILS(parseFloat(String(row.b2026).replace(/,/g,'')))}</td><td className="p-4 text-left font-black text-blue-800">{formatILS(executionMap[row.id] || 0)}</td></tr>
                    ))}</tbody>
                  </table>
                </div>
              )}
            ...
            <div className="space-y-8">
              {viewMode === 'dashboard' ? (
                <>
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white p-6 rounded-3xl border flex flex-col items-center"><CheckCircle2 className="text-emerald-500 mb-2" size={32}/><p className="text-[11px] font-black text-slate-400">בוצע</p><p className="text-3xl font-black text-slate-800">{workStats.p1}%</p></div>
                    <div className="bg-white p-6 rounded-3xl border flex flex-col items-center"><Clock className="text-amber-500 mb-2" size={32}/><p className="text-[11px] font-black text-slate-400">בתהליך</p><p className="text-3xl font-black text-slate-800">{workStats.p2}%</p></div>
                    <div className="bg-white p-6 rounded-3xl border flex flex-col items-center"><AlertCircle className="text-red-500 mb-2" size={32}/><p className="text-[11px] font-black text-slate-400">לא בוצע</p><p className="text-3xl font-black text-slate-800">{workStats.p3}%</p></div>
                    <div className="bg-white p-6 rounded-3xl border flex flex-col items-center"><HelpCircle className="text-slate-300 mb-2" size={32}/><p className="text-[11px] font-black text-slate-400">טרם עודכן</p><p className="text-xl font-black text-slate-400">{workStats.m} משימות</p></div>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[400px]">
                    <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                      <h3 className="font-black mb-4 border-r-4 border-emerald-500 pr-2">סטטוס ביצוע</h3>
                      <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={[{name:'בוצע', v:workStats.s1}, {name:'תהליך', v:workStats.s2}, {name:'טרם', v:workStats.s3+workStats.m}]} dataKey="v" innerRadius={60} outerRadius={90} paddingAngle={5}>{[ '#10b981', '#f59e0b', '#ef4444' ].map((c,i)=><Cell key={i} fill={c}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
                    </div>
                    <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm overflow-hidden">
                       <h3 className="font-black mb-4 border-r-4 border-blue-500 pr-2">משימות לפי מחלקה</h3>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-4 rounded-3xl border items-end">
                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 pr-2">חיפוש</label><input type="text" placeholder="חפש משימה..." value={search} onChange={e=>setSearch(e.target.value)} className="w-full p-2 bg-slate-50 border rounded-xl outline-none text-sm" /></div>
                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 pr-2">מחלקה</label>
                      <select value={filterDept} onChange={e=>setFilterDept(e.target.value)} className="w-full p-2 bg-slate-50 border rounded-xl outline-none text-sm" disabled={currentUser?.role === 'DEPT'}>
                        <option value="הכל">כל המחלקות</option>
                        {Array.from(new Set(workPlans.filter(t => !activeWingId || t.wing === activeWingId).map(t=>t.dept))).map(d => <option key={d} value={d}>{d}</option>)}
                      </select>
                    </div>
                    <button className="p-3 bg-slate-800 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2"><Download size={16}/> הורד אקסל</button>
                  </div>
                        ...
                        {filteredWorkData.map(t => (
                          <tr key={t.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-4 text-center">
                              ...
                              }} className={`p-1.5 rounded-xl text-[10px] font-black w-full outline-none border-none shadow-sm ${t.rating === 1 ? 'bg-emerald-100 text-emerald-700' : t.rating === 2 ? 'bg-amber-100 text-amber-700' : t.rating === 3 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-400'}`}>
