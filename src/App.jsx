    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-right overflow-x-hidden" dir="rtl">
      <header className="bg-white border-b sticky top-0 z-[250] shadow-sm p-4 flex justify-between items-center px-8">
          ...
          <div className="flex items-center gap-3">
            <div className="text-left hidden sm:block"><p className="text-[10px] font-black text-slate-600 leading-none">שלום, {currentUser.user}</p><button onClick={() => window.location.reload()} className="text-[10px] font-bold text-red-500 underline uppercase">התנתק</button></div>
            ...
            {mainTab === 'budget' ? (
              <div className="space-y-8">
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8 text-right">
                   <div className="bg-white p-4 rounded-2xl shadow-sm border"><p className="text-[8px] font-bold text-slate-400 uppercase">ביצוע 24</p><p className="text-lg font-black text-slate-700">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+c.a2024,0))}</p></div>
                   <div className="bg-white p-4 rounded-2xl shadow-sm border"><p className="text-[8px] font-bold text-slate-400 uppercase">תקציב 25</p><p className="text-lg font-black text-emerald-700">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+c.b2025,0))}</p></div>
                   <div className="bg-white p-4 rounded-2xl shadow-sm border-b-4 border-blue-600"><p className="text-[8px] font-bold text-slate-400 uppercase text-blue-600">תקציב 26</p><p className="text-lg font-black text-blue-800">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+c.b2026,0))}</p></div>
                   <div className="bg-white p-4 rounded-2xl shadow-sm border-b-4 border-orange-500"><p className="text-[8px] font-bold text-slate-400 uppercase text-orange-600">ביצוע+שריון</p><p className="text-lg font-black text-orange-700">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+c.commit2026,0))}</p></div>
                   <div className="bg-white p-4 rounded-2xl shadow-sm border-b-4 border-slate-400"><p className="text-[8px] font-bold text-slate-400 uppercase">ביצוע 26</p><p className="text-lg font-black text-slate-900">{formatILS(fullData.filter(r=>r.type==='הוצאה').reduce((a,c)=>a+c.a2026,0))}</p></div>
                </div>
                {viewMode === 'dashboard' ? (
                  ...
                  <div className="bg-white rounded-3xl shadow-sm border overflow-x-auto text-right p-4">
                    <table className="w-full text-right border-collapse min-w-[800px]">
                      <thead><tr className="bg-slate-50 text-[10px] font-black text-slate-400 border-b uppercase text-right text-right"><th className="p-4 pr-6 text-right">סעיף</th><th className="text-right">תיאור</th><th className="text-center text-right">מחלקה</th><th className="text-left text-right">תקציב 26</th><th className="text-left text-right">ביצוע 26</th></tr></thead>
                      <tbody className="divide-y">{fullData.filter(r=> (activeWingId === null || cleanStr(r.wing) === cleanStr(activeWingId)) && (search === '' || (r.name||"").includes(search) || String(r.id).includes(search))).map(row => (
                ...
                {viewMode === 'dashboard' ? (
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-4 rounded-3xl shadow-sm border items-end text-right">
                        ...
                        <div className="space-y-1 text-right"><label className="text-[10px] font-black text-slate-400 text-right">מחלקה</label><select value={filterDept} onChange={e => setFilterDept(e.target.value)} className="w-full p-2 lg:p-3 rounded-xl bg-slate-50 border outline-none font-bold text-sm text-right" disabled={currentUser?.role === 'DEPT'}><option value="הכל">כל המחלקות</option>{Array.from(new Set(filteredWorkData.map(t=>t.dept))).map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                        <div className="space-y-1 text-right"><label className="text-[10px] font-black text-slate-400 text-right">סטטוס</label><select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full p-2 lg:p-3 rounded-xl bg-slate-50 border outline-none font-bold text-sm text-right"><option value="הכל">כל הסטטוסים</option><option value="1">בוצע</option><option value="2">בתהליך</option><option value="3">לא בוצע</option><option value="missing">טרם עודכן</option></select></div>
                    </div>
                    <div className="bg-white rounded-[2.5rem] shadow-sm border overflow-x-auto text-right">
